...
import type { GarnishSpec, GlassName, GlassSpec } from './glass-svg';
...
/** A lighter wash toward white — used for card / hero backgrounds. Higher `amount` = paler. */
export function washFor(c: Cocktail, amount = 0.74): string {
  return mix(liquidFor(c), '#ffffff', amount);
}
