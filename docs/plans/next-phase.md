...
> Cloudflare Tunnel
> **terminates TLS at the edge** — inherent to how WAF, Access and caching work. A Barkaart login
...
```bash
age -d -i <key> backups/barkaart-<stamp>.archive.gz.age | docker compose exec -T mongo \
  mongorestore -u "$MONGO_USER" -p "$MONGO_PASSWORD" --authenticationDatabase admin \
  --archive --gzip --nsFrom 'barkaart.*' --nsTo 'restoretest.*'
```
...
- ~~**The "Welke Cocktail" rebrand**~~ **Superseded — the app is now Barkaart** (branch `rename/barkaart`). The name moved from what you *put in* (your cabinet) to what you *get out* (your menu); `origin/claude/project-hosting-plan-nwznl8` and its "Welke Cocktail" naming are dead. The churn that entry warned about is real and only half-paid: the repository, the SPA and the localStorage namespace are renamed (with a copy-forward migration in `frontend/src/app/core/storage-migration.ts`), but the **running box still holds the old Mongo user/db, image tag, container and tunnel names** — see the migration section in [`deploy/README.md`](../../deploy/README.md). Do that before the next deploy, not during sync work.
...
- ~~**New staples never reach an existing cabinet.**~~ **Done** — the migration option, not the
  ...
  (`barkaart.staplesApplied`) and unions anything added since into a *non-empty* cabinet on load; an
